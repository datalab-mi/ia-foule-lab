...
class Dsnet:
    """Class for DSNet Inference (Crowd Counting Model Inference)"""
    ...
    def __init__(self, model_path=None):
        ...
        if self.model_type != 'dsnet':

            self.error_message = "The model is not a 'DSNet' model : {}".format(self.model_filename)

        elif self.model_extension not in Dsnet.authorized_extension:

            self.error_message = "'DSNet' extension not yet implemented : {}".format(self.model_extension)
            return
